...
CACHES = {
    "KAN-Fourier": RESULTS / "kan_fourier_ablation_full_mseed261_predictions.npz",
    "Real-valued multiscale conv": RESULTS
    / "kan_fourier_ablation_full_geo_2expert_no_quaternion_mseed261_predictions.npz",
    "First-order pooling": RESULTS
    / "kan_fourier_ablation_full_geo_2expert_no_spd_mseed261_predictions.npz",
    "Parameter-matched MLP": RESULTS
    / "kan_fourier_ablation_full_geo_2expert_no_fourier_kan_mseed261_predictions.npz",
}

OUTPUT_PNG = FIGURES / "fig_10a_ablation.png"
OUTPUT_CSV = FIGURES / "fig_10a_ablation.csv"
...
def main() -> None:
    ...
    for name in ("Real-valued multiscale conv", "First-order pooling"):
        ...
        ax1.plot(
            snrs,
            [values[s] - base_curve[s] for s in snrs],
            markersize=5.2,
            markerfacecolor="white",
            markeredgewidth=1.35,
            label=name,
    ...
    ax1.set_yticks(np.arange(-4, 2.1, 1))
    ...
    print("Alignment check passed for all component-ablation predictions.")
    for name, cache in caches.items():
