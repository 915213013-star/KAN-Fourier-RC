...
def checkpoint_paths(args, fold):
    suffix = f"fourier_main_oof_mseed{args.model_seed}_fold{fold}_split{args.split_seed}"
    return (
...
def save_checkpoint(path, model, optimizer, scheduler, epoch, best_score, best_metrics, args, fold):
    torch.save(
        {
            "epoch": int(epoch),
            ...
            "fold": int(fold),
            "model_type": "fourier_main_oof_2016",
            "args": vars(args),
        },
        path,
    )
...
def train_fold(args, fold, train_loader, holdout_loader, device):
    ...
    resume_best_reset = bool(getattr(args, "resume_best_reset", False))
    if resume_best_reset and os.path.exists(best_path) and not args.force_restart:
        ...
        print(
            f"[*] Warm restart Fourier OOF fold {fold} from best checkpoint: "
            f"start_epoch=1, best_score={best_score:.4f}"
        )
    elif os.path.exists(latest_path) and not args.force_restart:
        ...
        best_score = float(ckpt.get("best_score", best_score))
        best_metrics = ckpt.get("best_metrics", None)
        print(f"[*] Resume Fourier OOF fold {fold}: start_epoch={start_epoch}, best_score={best_score:.4f}")
    ...
    for epoch in range(start_epoch, int(args.epochs) + 1):
        ...
        m = metrics_from_probs(prob, labels, snrs)
        score = common.score_metrics(m)
        print(
            f"\nFourier OOF fold {fold} | Epoch {epoch:03d}/{args.epochs} | {time.time() - t0:.1f}s | "
            f"loss={train_m['loss']:.4f} acc={train_m['acc']:.2f}% skipped={train_m['skipped']}"
        )
        common.print_metrics("Holdout", m, score)

        save_checkpoint(latest_path, model, optimizer, scheduler, epoch, best_score, best_metrics, args, fold)
        if score > best_score:
            best_score = score
            best_metrics = m
            stale = 0
            save_checkpoint(best_path, model, optimizer, scheduler, epoch, best_score, best_metrics, args, fold)
            print(f"[*] New Fourier fold best saved: {best_path}")
        else:
            stale += 1
...
def main():
    ...
    if not args.output_cache:
        args.output_cache = common.relpath(
            "results",
            f"fourier_main_oof_mseed{args.model_seed}_f{args.folds}e{args.epochs}_split{args.split_seed}_trainvaltest_probs_for_meta.npz",
        )
    ...
    print("  - Only original train split is cross-fitted.")
    print("  - Every train OOF prediction comes from a model that did not see that sample.")
    print("  - Validation is used only for fold checkpoint selection.")
    print("  - Test probabilities are exported for final fusion; test accuracy is not printed here.")
    ...
    for fold, (tr, va) in enumerate(skf.split(train_idx, composite), 1):
        ...
        holdout_loader = make_loader(full_dataset, holdout_idx, args.eval_batch_size, False, args.num_workers, device)
        ...
        common.print_metrics(f"Fold {fold} Holdout", metrics_from_probs(ph, yh, sh), best_score)
        common.print_metrics(f"Fold {fold} Val", metrics_from_probs(pv, yv, sv), common.score_metrics(metrics_from_probs(pv, yv, sv)))
        val_probs.append(pv)
    ...
    np.savez_compressed(
        ...
        protocol=np.asarray(["train-split OOF original Fourier/KAN MoE main model; test probabilities exported without scoring"]),
    )
