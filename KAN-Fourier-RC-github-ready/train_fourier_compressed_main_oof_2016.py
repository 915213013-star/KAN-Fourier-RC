"""Leakage-controlled OOF training for the compact KAN-Fourier variants.

Epoch selection is performed on an inner split of each outer-training fold.
The selected weights are discarded, and a freshly initialized model is then
refit on the complete outer-training fold for the selected number of epochs.
Only that refitted model is allowed to infer the outer holdout OOF rows.
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Train a compact KAN-Fourier model with inner selection and fresh outer-fold refitting."
    )
    ...
    # Accepted only to fail loudly when an old command is reused. These legacy
    # controls can make a fold depend on its own holdout diagnostics.
...
def _reject_legacy_controls(args):
    ...
    if rejected:
        raise ValueError(
            "The corrected public OOF protocol rejects holdout-dependent legacy controls: "
            + ", ".join(rejected)
...
def _train_member(args, fold, full_dataset, outer_train_idx, outer_holdout_idx, device):
...
def main_with_args(args, title="Train compact KAN-Fourier OOF cache"):
    ...
    print("  - Inner-selection labels choose only the training horizon.")
    print("  - Selection weights are discarded before fresh outer-training-fold refitting.")
    print("  - Outer-holdout labels never select checkpoints, member seeds, weights, or hyperparameters.")
    print("  - The same predeclared seed list and soup weights are used for every outer fold.")
    print("  - Validation/test probabilities are exported unscored.")
    print(
    ...
    for fold, (outer_train_pos, outer_holdout_pos) in enumerate(splitter.split(train_idx, composite), 1):
        ...
        for member_seed in fixed_member_seeds:
            ...
            model, selection_score, selected_epoch, refit_meta = _train_member(
                args,
                fold,
                full_dataset,
                outer_train_idx,
    ...
    protocol_summary.update(
        {
            ...
            "legacy_holdout_dependent_controls": "rejected",
        }
    )
    np.savez_compressed(
        args.output_cache,
        ...
        protocol=np.asarray(
            [
                "inner epoch selection followed by fresh complete outer-training-fold refit; "
                "outer holdout inferred once; validation/test probabilities exported unscored"
            ]
        ),
    )
