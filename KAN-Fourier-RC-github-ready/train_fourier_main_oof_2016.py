...
def train_fold(args, fold, full_dataset, outer_train_idx, outer_holdout_idx, device):
    """Select an epoch internally, then refit before touching the outer holdout."""
    ...
    oofp.assert_partition_invariants(outer_train_idx, inner_train_idx, inner_select_idx, outer_holdout_idx)
    print(
    ...
    inner_train_loader = make_loader(
        full_dataset, inner_train_idx, args.batch_size, True, args.num_workers, device, drop_last=True
    )
    inner_select_loader = make_loader(
        full_dataset, inner_select_idx, args.eval_batch_size, False, args.num_workers, device
    )
    ...
    model, optimizer, scheduler = _new_training_state(args, device, inner_train_loader, args.epochs)
    ...
    if not args.force_restart:
        ...
        if latest is not None:
            ...
            if bool(latest.get("selection_complete", False)):
                ...
                print(f"[*] Reusing completed inner selection for fold {fold}: epoch={selected_epoch}")
            else:
    ...
    if not selection_complete:
        for epoch in range(start_epoch, int(args.epochs) + 1):
            ...
            train_m = train_one_epoch(model, inner_train_loader, optimizer, scheduler, ce, supcon, device, args)
            prob, labels, snrs = collect_probs(model, inner_select_loader, device)
            metrics = metrics_from_probs(prob, labels, snrs)
            score = common.score_metrics(metrics)
            print(
                f"\nFourier inner selection fold {fold} | Epoch {epoch:03d}/{args.epochs} | "
        ...
        save_checkpoint(
            ...
            optimizer,
            scheduler,
            int(selected.get("epoch", selected_epoch)),
            best_score,
            best_metrics,
            args,
            fold,
    ...
    # A fresh initialization is mandatory here. Selection weights are discarded.
    del model, optimizer, scheduler
    outer_train_loader = make_loader(
        full_dataset, outer_train_idx, args.batch_size, True, args.num_workers, device, drop_last=True
    )
    ...
    best_path, latest_path = checkpoint_paths(args, fold)
    set_seed(int(args.model_seed) + 1000 * int(fold))
    model, optimizer, scheduler = _new_training_state(args, device, outer_train_loader, selected_epoch)
    start_epoch = 1
    ...
    print(f"[*] Fresh outer refit fold {fold}: train all {len(outer_train_idx):,} rows for {selected_epoch} epochs")
    for epoch in range(start_epoch, selected_epoch + 1):
        t0 = time.time()
        setattr(args, "_current_epoch", int(epoch))
        train_m = train_one_epoch(model, outer_train_loader, optimizer, scheduler, ce, supcon, device, args)
        print(
            f"Fourier outer refit fold {fold} | Epoch {epoch:03d}/{selected_epoch} | "
            f"{time.time() - t0:.1f}s | loss={train_m['loss']:.4f} "
            f"acc={train_m['acc']:.2f}% skipped={train_m['skipped']}"
        )
        save_checkpoint(
...
def main():
    ...
    print("  - Selection weights are discarded; a fresh model is refit on the complete outer-training fold.")
    print("  - Outer-holdout labels never select an epoch, checkpoint, or hyperparameter.")
    print("  - Outer-holdout probabilities are generated once after refitting and become the OOF rows.")
    print("  - Validation/test probabilities are exported unscored for downstream frozen evaluation.")
    ...
    np.savez_compressed(
        ...
        protocol=np.asarray(
            [
                ...
                "outer holdout used once for OOF inference; validation/test probabilities exported unscored"
            ]
        ),
    )
